  ...
  const dropdownContent = dateOpen && (
    <>
      ...
      >
        ...
        <div
          className={cn(
            'px-5 py-4 border-b border-slate-100 bg-linear-to-r from-slate-50 to-[#e8f8f5]/30 flex items-center justify-between sticky top-0 z-10',
            isMobile && 'px-6 py-5'
          )}
        >
          <div>
            <h3 className="font-semibold text-slate-800">Select Date</h3>
            <p className="text-xs text-slate-500 mt-0.5">When do you want to travel?</p>
          </div>
          {isMobile && (
            <button
              onClick={() => setDateOpen(false)}
    ...
    <div className={cn('h-full relative', className)}>
      ...
      >
        {mounted && date ? (
          ...
          <span className="text-sm lg:text-base text-slate-400">Select travel date</span>
        )}
      </div>
