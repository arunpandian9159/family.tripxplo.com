...
export function generateShareMessage(options: ShareMessageOptions): string {
  ...
  const destination =
    extractDestination(packageName) ||
    (location ? extractDestination(location) : null);
  ...
  const headerEmoji =
    destination?.emoji?.charAt(0) || theme?.emoji?.charAt(0) || "✈️";
  ...
  if (destination || theme) {
    const feature =
      destination?.feature || theme?.tagline || "Amazing Experience";
    lines.push(`✨ ${feature}`);
  }
