...
import { indianHolidays, type Holiday } from '@/lib/indian-holidays';
...
export function HolidayCalendar({ selected, onSelect, disabled, className }: HolidayCalendarProps) {
        ...
        week.push(
          <button
            ...
            className={cn(
              ...
              // Holiday styling - using teal color scheme
              holiday && isCurrentMonth && 'bg-[#e8f8f5] hover:bg-[#d1fbd2]',
              // Selected state
              isSelected && !holiday && 'bg-[#15ab8b] text-white hover:bg-[#0f8a6f]',
              isSelected && holiday && 'ring-2 ring-[#15ab8b] ring-offset-1',
              // Today indicator
          ...
          >
            ...
            <span
              className={cn(
                ...
                holiday && isCurrentMonth && 'text-[#15ab8b]',
            ...
            {holiday && isCurrentMonth && (
              <span
                className={cn(
                  'text-[9px] leading-[1.2] text-[#0f8a6f] font-medium mt-0.5 line-clamp-3 w-full wrap-break-word hyphens-auto',
                  isSelected && 'text-[#064e3b]'
                )}
              >
                {holiday.name}
              </span>
  ...
  return (
    <div className={cn('w-auto select-none', className)}>
      ...
      <div className="flex justify-between gap-0.5 mb-2 px-0.5">
        {weekDays.map(day => (
          <div key={day} className="w-[56px] text-center text-xs font-semibold text-slate-500">
            {day}
          </div>
        ))}
      </div>
