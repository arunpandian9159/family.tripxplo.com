...
// GET /api/v1/destinations/search - Search destinations (tripxplo.com compatible)
export async function GET(request: NextRequest) {
  try {
    ...
    const searchFilter = query ? { destinationName: { $regex: query, $options: 'i' } } : {};

    const [destinations, total] = await Promise.all([
      Destination.find(searchFilter).sort({ rankNo: -1 }).skip(skip).limit(limit).lean(),
