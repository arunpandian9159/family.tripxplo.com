  ...
  try {
    ...
    if (availableVehicleIds.length === 0) {
      ...
      if (vehicleIds.length === 0) {
        return successResponse([{ vehicleDetails: [] }], 'No available vehicles');
      }

      const vehicles = await Vehicle.find({ vehicleId: { $in: vehicleIds } });
      return successResponse([{ vehicleDetails: vehicles }], 'Vehicles retrieved successfully');
    }

    // Get all available vehicles
    const vehicles = await Vehicle.find({ vehicleId: { $in: availableVehicleIds } });
