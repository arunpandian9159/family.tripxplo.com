...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!packageData) {
      return errorResponse('Package data is required', ErrorCodes.VALIDATION_ERROR, 400);
    }
      ...
      totalActivityPrice = (packageData.activity || []).reduce(
        (total: number, act: ActivityItem) => {
          const eventTotal = (act.event || []).reduce(
            (evtTotal: number, evt: ActivityEvent) => evtTotal + (evt.price || 0),
            0
          );
    ...
    const totalAdditionalFee =
      (packageData as any).totalAdditionalFee ?? packageData.additionalFees ?? 0;
    ...
    const totalTransportFee = (packageData as any).totalTransportFee ?? packageData.transPer ?? 0;
    ...
    const totalAdultCount = packageData.totalAdultCount || packageData.noOfAdult || 2;
