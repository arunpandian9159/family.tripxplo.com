...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const hotelDetails = await Hotel.find({
      hotelId: { $in: hotelRoomData.map(r => r.hotelId) },
    }).lean();
    ...
    (pkg.hotel || []).forEach(
      (hotel: {
        ...
        let selectedMealPlan = roomData.mealPlan?.find(
          (mp: { mealPlan: string; startDate: string[]; endDate: string[]; seasonType: string }) =>
            mp.mealPlan === hotel.mealPlan && startDate && getSeasonType(startDate, mp)
        );
        if (!selectedMealPlan)
          selectedMealPlan = roomData.mealPlan?.find(
            (mp: { mealPlan: string }) => mp.mealPlan === hotel.mealPlan
          );
        ...
        hotelMeal.push({
          ...
          startDatePrice: selectedMealPlan.startDate,
          endDatePrice: selectedMealPlan.endDate,
          totalAdultPrice,
          gstAdultPrice,
          totalChildPrice,
          gstChildPrice,
