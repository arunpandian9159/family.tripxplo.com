    ...
    try {
      ...
      if (!response.ok && !isPackageId(identifier)) {
        ...
        if (fallbackData.success && fallbackData.data?.result) {
          return { result: Array.isArray(fallbackData.data.result) ? fallbackData.data.result : [fallbackData.data.result] };
        }
        if (fallbackData.success && fallbackData.result) {
          return { result: Array.isArray(fallbackData.result) ? fallbackData.result : [fallbackData.result] };
        }
        if (fallbackData.result) {
          return { result: Array.isArray(fallbackData.result) ? fallbackData.result : [fallbackData.result] };
        }
        return { result: Array.isArray(fallbackData) ? fallbackData as PackType[] : [fallbackData as PackType] };
      }
...
const packageSlice = createSlice({
  ...
  reducers: {
    ...
    changeActivity: (
      ...
      action: PayloadAction<changeActivityType>
    ) => {
      state.isLoading = true;
    ...
    changeVehicle: (
      state: PackageSliceType,
      action: PayloadAction<ChangeVehicleType>
    ) => {
      state.isLoading = true;
      const { prevVehicle, newVehicle } = action.payload;

      const updatedVehicles = state.data?.vehicleDetail?.map(
        (vehicle): Vehicle => {
...
export const changeActivityAndCalculatePrice = (
  payload: changeActivityType
) => {
