...
const CabInfoList = ({ cab, prevCab }: { cab: VehicleDetail; prevCab: VehicleDetail }) => {
  ...
  return (
    <div className=" rounded-3xl border-2 border-emerald-100 bg-white p-4 max-w-[457px] min-h-[280px] sm:min-h-[315px]  my-2">
      <div className="h-full flex flex-col">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-changeHotelLine p-2 sm:p-4 flex-1">
          ...
          <div className="w-full sm:w-1/2 space-y-4">
            <div>
              ...
              <p className="text-emerald-500 text-sm sm:text-base">{cab.vehicleCompany}</p>
            </div>

            <div>
              <p className="text-[#FF5F5F] mb-2 sm:mb-4 text-[11px] sm:text-[12px] line-clamp-2">
                {cab?.transferInfo?.join('')?.replace('Sightseeing', 'Sightseeing, ')}
              </p>
              <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                <div className="flex items-center gap-1 sm:gap-2">
                  <RockingChair className="text-[#FF5F5F] text-xs" />
                  <span className="text-gray-600 text-xs">{cab.seater} Seater</span>
                </div>
                <div className="flex items-center gap-1 sm:gap-2">
                  <Luggage className="text-[#FF5F5F] text-xs" />
                  <span className="text-gray-600 text-xs">{cab.luggage} Luggage</span>
                </div>
              </div>
            </div>
          </div>
