...
const Page = () => {
  ...
  const handleShare = async () => {
    ...
    if (searchParams?.noOfAdult) params.set('adults', searchParams.noOfAdult.toString());
    if (searchParams?.noRoomCount) params.set('rooms', searchParams.noRoomCount.toString());
  ...
  const totalActivities = activity?.reduce((acc, day) => acc + (day?.event?.length || 0), 0) || 0;
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 pb-16 pt-10 px-4 md:px-6 bg-linear-to-t from-black/90 via-black/60 to-transparent">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-lg mb-3">
              ...
              <span className="text-white text-sm font-semibold">Full Itinerary</span>
            </div>
          ...
          <div className="relative mt-8">
            ...
            {activity?.length > 0 ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-slate-500 text-sm">Your itinerary activities will appear here</p>
              </div>
            )}
          </div>
        </div>
