...
export default function HotelData({ hotel, index }: { hotel: HotelMeal; index: number }) {
  ...
  const [selectedNewHotel, setSelectedNewHotel] = useState<HotelChangeDataType | null>(null);
      ...
      <div className="group bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden">
        ...
        <div className="p-5">
          <div className="flex flex-col md:flex-row gap-5">
            {/* Image */}
            <div className="relative w-full md:w-56 h-40 md:h-44 rounded-xl overflow-hidden shrink-0 group/image">
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              <h4 className="text-xl font-bold text-slate-900 mb-1 line-clamp-1">{hotelName}</h4>
                  ...
                  <span className="text-xs font-medium text-blue-700">
                    {(() => {
                      try {
                        ...
                        const searchDateStr = useSelector((state: any) => state.searchPackage.date);
