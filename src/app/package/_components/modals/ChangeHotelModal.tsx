...
/**
 * Filters hotel rooms to only include those with valid meal plans for the selected date.
 * @param hotel - The hotel with rooms
 * @param checkDate - The date to check
 * @returns A new hotel object with filtered rooms, or null if no valid rooms exist
 */
const filterHotelRoomsByDate = (
  hotel: HotelChangeDataType,
  checkDate: string
): HotelChangeDataType | null => {
  if (!hotel?.hotelRoom?.length) return null;

  const filteredRooms = hotel.hotelRoom
    .map(room => filterRoomMealPlansByDate(room, checkDate))
...
const ChangeHotelModal: React.FC<ChangeHotelModalProps> = ({
  ...
  const processedHotels = useMemo(() => {
    // First, filter hotels by date - only keep hotels with rooms that have valid meal plans for the selected date
    const dateFilteredHotels = (availableHotels as HotelChangeDataType[])
      .map(h => {
        ...
        return filterHotelRoomsByDate(h, effectiveDate);
      })
      .filter((h): h is HotelChangeDataType => h !== null);

    // Then apply the meal plan compatibility filter
    let hotels = dateFilteredHotels.filter(h => {
    ...
    return hotels;
  }, [availableHotels, hotel?.mealPlan, searchQuery, starRating, userRating, sortBy, searchDate]);
