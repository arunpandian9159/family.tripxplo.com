...
const CabCard = ({
  ...
  const typeColor = getVehicleTypeColor(cab?.vehicleName || cab?.vehicleCompany || '');
  ...
  return (
    <div className={`relative bg-white rounded-2xl border overflow-hidden transition-all duration-300 ${isSelected
      ...
      {/* Vehicle Image */}
      <div className="relative h-32 bg-linear-to-br from-slate-100 to-slate-50 overflow-hidden">
        {cab?.image ? (
          <Image
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          ...
          <div className="flex flex-col items-center p-2 bg-slate-50 rounded-lg">
            ...
            <span className="text-xs font-bold text-slate-700">{cab?.luggage || '-'}</span>
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {cab?.acType && (
            <div className="flex items-center gap-1 px-2 py-1 bg-cyan-50 rounded-full">
              ...
              <span className="text-[10px] text-cyan-600 font-medium">{cab.acType}</span>
            </div>
          )}
          {cab?.maxPax && cab.maxPax > 0 && (
            <div className="flex items-center gap-1 px-2 py-1 bg-slate-50 rounded-full">
              <Users size={10} className="text-slate-400" />
              <span className="text-[10px] text-slate-500 font-medium">Max {cab.maxPax} pax</span>
            </div>
          )}
          <div className="flex items-center gap-1 px-2 py-1 bg-emerald-50 rounded-full">
            <Shield size={10} className="text-emerald-500" />
            <span className="text-[10px] text-emerald-600 font-medium">Insured</span>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-y-auto p-6 max-h-[calc(90vh-80px)]">
          {isLoading || loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 bg-white rounded-2xl">
              ...
              <p className="text-slate-600 font-semibold">No Vehicles Available</p>
