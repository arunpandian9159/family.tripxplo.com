...
const PackageDetail = ({ pack }: PropsType) => {
  ...
  if (!pack || !pack.packageId) {
    return (
      <div className="bg-linear-to-br from-slate-50 to-white min-h-screen flex items-center justify-center">
  ...
  const getMealPlanInfo = () => {
    const mealPlans = pack?.hotelMeal?.map(hotel => hotel?.mealPlan?.toLowerCase()) || [];
    const hasBreakfast = mealPlans.some(mp => mp === 'cp' || mp === 'map' || mp === 'ap');
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                ...
                {hasAllMeals ? (
                  <div className="inline-flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-lg">
                    ...
                    <span className="text-sm font-medium text-emerald-700">All Meals Included</span>
                  </div>
                ) : (
                  <>
                    {hasBreakfast && (
