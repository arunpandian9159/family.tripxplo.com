...
const PackageHighlight = ({
  ...
  const stats = [
    ...
    },
    {
      icon: Users,
      label: 'Travelers',
      value: `${totalAdult} Adult${totalAdult > 1 ? 's' : ''}${totalChild > 0 ? `, ${totalChild} Child${totalChild > 1 ? 'ren' : ''}` : ''}`,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <span className={`inline-flex items-center gap-1.5 px-4 py-2 bg-linear-to-r ${planGradient} text-white text-sm font-semibold rounded-full shadow-md`}>
          <Sparkles size={14} />
          {plan} Plan
        </span>
      </div>
      ...
      {safeDestinations.length > 0 && (
        <div className="flex items-start gap-3 p-4 bg-linear-to-r from-slate-50 to-white rounded-2xl border border-slate-100">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-2">Destinations</p>
                ...
                .map((dest, i, arr) => (
                  <React.Fragment key={dest?._id || i}>
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-white rounded-lg border border-slate-100 shadow-sm">
                      <span className="text-coral-500 font-bold text-sm">{dest?.noOfNight}N</span>
                      <span className="text-slate-700 font-medium text-sm">{dest?.destinationName || 'Destination'}</span>
                    </div>
                    {i !== arr.length - 1 && (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, index) => (
          ...
          >
            <div className={`inline-flex p-2.5 ${stat.bgColor} rounded-xl mb-3 group-hover:scale-110 transition-transform duration-300`}>
