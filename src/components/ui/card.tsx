...
export function CardTitle({ children, className, as: Tag = 'h3' }: CardTitleProps) {
  return <Tag className={cn('text-lg font-semibold text-slate-900', className)}>{children}</Tag>;
}
  ...
  return (
    <Card className={cn('', className)}>
      <div className="flex items-start justify-between">
        ...
        {icon && <div className="p-3 rounded-xl bg-slate-50 text-slate-600">{icon}</div>}
      </div>
    </Card>
  );
}
