...
export const getPackages = async (payload: GetPackageQueryType) => {
  try {
    ...
    if (payload.noRoomCount !== undefined)
      params.noRoomCount = payload.noRoomCount;
    if (payload.noExtraAdult !== undefined)
      params.noExtraAdult = payload.noExtraAdult;
    if (payload.limit) params.limit = payload.limit;
    if (payload.offset !== undefined) params.offset = payload.offset;
    if (payload.priceOrder !== undefined)
      params.priceOrder = payload.priceOrder;
    ...
    if (response.success && response.data) {
      ...
      return {
        result: {
          ...
          hasNextPage:
            data.hasNextPage ??
            (data.page ? data.page < (data.totalPages || 1) : false),
