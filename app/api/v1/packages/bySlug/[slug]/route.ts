...
// GET /api/v1/packages/bySlug/:slug - Get package details by slug with price calculation
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const hotelsParam = url.searchParams.get("hotels"); // Format: "index:hotelId,index:hotelId"
    const requestedHotels: Record<number, string> = {};

    if (hotelsParam) {
      hotelsParam.split(",").forEach((part) => {
        const [indexStr, hotelId] = part.split(":");
    ...
    // First get the basic package data by slug
    ...
    const hotelRoomIds = (pkg.hotel || []).map(
      (h: { hotelRoomId: string }) => h.hotelRoomId,
    );
    let hotelRoomData = await HotelRoom.find({
    ...
    (pkg.hotel || []).forEach((hotel: any, index: number) => {
      ...
      if (requestedHotels[index]) {
        ...
        // Try to find a room for this hotel
        // Prefer a room that matches the original room type if possible, or just the first available room
        const currentRoom = hotelRoomData.find(
          (r: any) => r.hotelRoomId === activeHotelRoomId,
        );

        const bestReplacement = hotelRoomData.find((r: any) => {
          if (r.hotelId !== wantedHotelId) return false;
          // Try to match room type if we can
          if (currentRoom && r.hotelRoomType === currentRoom.hotelRoomType)
            return true;
    ...
    // Get vehicle details
    const vehicleDetail = (pkg.vehicleDetails || []).map(
      (v: {
        vehicleId: string;
        vehicleName: string;
        price: number;
    ...
    const activityWithDates = (pkg.activity || []).map(
      (act: {
        ...
        return {
          ...act,
          fullStartDate: actDate.toLocaleDateString("en-US", {
            day: "2-digit",
            month: "short",
          }),
        };
