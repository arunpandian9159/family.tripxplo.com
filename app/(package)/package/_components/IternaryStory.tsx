            ...
            <div className="space-y-6">
              ...
              {[...activities]
                ...
                .map((dayActivity) => {
                  ...
                  const events = Array.isArray(dayActivity.event)
                    ? dayActivity.event
                    : [];

                  return (
                    <div
                      key={dayActivity._id || dayActivity.day}
                      className="relative"
                    >
                      {/* Day Header */}
                      <div className="flex items-center gap-3 mb-4">
                        <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-emerald-500 to-teal-500 text-white text-sm font-bold rounded-xl shadow-md shadow-emerald-500/25">
                          {dayActivity.day || 1}
                        </div>
                        <div>
                        ...
                        {events.slice(0, 3).map((event, eventIndex) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                className={`p-2 ${config.bg} rounded-lg flex-shrink-0`}
                              >
                                <IconComponent
                                  size={16}
              ...
              {activities.length > 2 && (
                <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <span className="text-sm text-slate-500">
                    +{activities.length - 2} more day
                    {activities.length - 2 !== 1 ? "s" : ""} in your itinerary
                  </span>
                </div>
              )}
            </div>
