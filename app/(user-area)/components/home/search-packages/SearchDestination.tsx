...
const FEATURED_DESTINATIONS = [
  ...
  { name: "Varkala", tag: "IN SEASON", color: "bg-pink-100 text-pink-800" },
  ...
  //{ name: 'Thailand', tag: 'HONEYMOON', color: 'bg-pink-100 text-pink-800' },
];
...
const SearchDestination = ({
  ...
  // Filter to only show featured destinations
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3">
          <MapPin className="w-4 h-4 text-gray-400 group-hover:text-coral-500 transition-colors" />
    ...
    <>
      {isLoading ? (
        <div className="px-4 py-8 text-center">
          <Loader2 className="w-6 h-6 text-coral-500 animate-spin mx-auto mb-2" />
          ...
          {domesticDestinations.length > 0 && (
            <div>
              ...
              <div className="space-y-1">
                {domesticDestinations.map(renderDestinationItem)}
              </div>
            </div>
          )}
  ...
  const dropdownContent = isInputFocused && (
    <>
      ...
      >
          ...
          <div className="px-5 py-3 bg-slate-50 border-b border-slate-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-base outline-none focus:border-coral-400 focus:ring-2 focus:ring-coral-100 transition-all"
              />
            </div>
          </div>
        )}
        ...
        {/* Destinations list */}
        <div className={cn(isMobile && "max-h-[60vh] overflow-y-auto")}>
          {destinationsContent}
        </div>
      </div>
    </>
