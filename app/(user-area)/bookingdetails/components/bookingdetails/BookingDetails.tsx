    ...
    <div className="animate-fade-in">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-4 border border-slate-100 shadow-sm">
            <div className="flex flex-col md:flex-row gap-8">
              <div className="relative w-full md:w-64 h-56 rounded-[2rem] overflow-hidden shadow-2xl shadow-slate-200">
                ...
                <div
                  className={cn(
                    "absolute top-4 left-4 px-4 py-1.5 rounded-full text-white text-[10px] font-black uppercase tracking-widest shadow-lg backdrop-blur-md",
                    status?.bg
                  )}
                >
                  {pack?.status}
              ...
              <div className="flex-1 py-4 pr-4">
                ...
                <div className="grid grid-cols-3 gap-6 mt-8 pt-8 border-t border-slate-50">
                  ...
                  <div>
                    ...
                    >
                      <p className="text-lg font-bold text-gold-600 group-hover:underline">
                        {pack?.bookingId?.slice(0, 8)}
                      </p>
                      <Copy
                        size={12}
                        className="text-slate-300 group-hover:text-gold-500"
                      />
                    </div>
                  </div>
                </div>
                      ...
                      {emiSchedule.map((item: any, i: number) => (
                        ...
                        >
                          <td className="px-8 py-6 text-sm font-black text-slate-300 group-hover:text-slate-900">
                            {item.installmentNumber.toString().padStart(2, "0")}
                          </td>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-white font-black text-lg">
                        Next EMI # {nextPendingEmi.installmentNumber}
                      </p>
                      <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">
                        Due on{" "}
                        {new Date(nextPendingEmi.dueDate).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={handlePayEmi}
                    className="w-full sm:w-auto px-10 py-4 gold-gradient text-white text-sm font-black rounded-2xl shadow-xl shadow-gold-500/20 hover:scale-105 active:scale-95 transition-all uppercase tracking-widest"
                  >
          ...
          {/* Package details: Hotels, Itinerary, Transfers, etc. */}
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center px-4">
                    ...
                    <span className="text-emerald-600 font-black">
                      {formatIndianCurrency(paidEmis * emiAmount)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center px-4">
                    <span className="text-slate-500 text-xs font-bold uppercase">
                    ...
                    <span className="text-gold-600 font-black">
                      {formatIndianCurrency(
                        totalEmiAmount - paidEmis * emiAmount
                      )}
                    </span>
                  </div>
                  ...
                  <div className="p-5 bg-slate-900 rounded-[2rem] text-center shadow-xl shadow-slate-200 relative overflow-hidden group">
                    ...
                    <p className="text-gold-400 text-[10px] font-black uppercase tracking-widest mb-1">
                      Current Monthly
                    </p>
                    ...
                    <button
                      onClick={handlePayEmi}
                      className="mt-4 w-full py-3 bg-gold-500 text-white text-xs font-black uppercase rounded-xl hover:bg-gold-600 transition-colors shadow-lg shadow-gold-500/20"
                    >
                      Pay Now
                    </button>
                  </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm">
              ...
              <div className="relative pl-6">
                {events.map((event: any, index: number) => (
                  <div
                    key={index}
                    className="flex gap-6 pb-12 relative group last:pb-0"
                  >
                    {!(index === events.length - 1) && (
                      <div
                        className={cn(
                          "absolute left-[11px] top-8 w-[2px] h-full transition-all duration-500",
                    ...
                    <div
                      className={cn(
                        "relative z-10 w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 border-4 border-white shadow-md transition-all duration-500",
                        event.status === "completed"
                          ? "bg-gold-500 scale-125 shadow-gold-500/30"
                          : "bg-slate-200"
                      )}
                    >
                      {event.status === "completed" && (
            ...
            <div className="mt-10 pt-8 border-t border-slate-50">
              <div className="p-6 bg-red-50/50 rounded-[2rem] border border-red-100/50">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h5 className="text-sm font-black text-red-900 uppercase tracking-tighter">
                    Cancellation Policy
                  </h5>
                </div>
                <p className="text-[11px] text-red-700/80 font-bold leading-relaxed mb-6">
                  Standard 20% processing fee applied on refunded amount for
                  cancellations made before travel.
                </p>
